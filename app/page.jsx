  ...
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .gc {
      ...
      transition:border-color 0.35s, box-shadow 0.35s, transform 0.35s, background 0.4s;
      overflow:hidden;
    }
    .gc::before {
    ...
    .btn-g:hover { border-color:rgba(59,158,255,0.4); color:var(--text); background:var(--surface-2); }
    ...
    nav.scrolled { background:var(--nav-bg); backdrop-filter:blur(24px); border-color:var(--border); }
    .nav-logo { font-family:var(--font-mono); font-size:15px; font-weight:700; color:var(--text); text-decoration:none; transition:color 0.3s; }
    ...
    @media(max-width:768px){
      ...
      .nav-links.open{display:flex;flex-direction:column;position:fixed;top:61px;left:0;right:0;background:var(--mobile-menu-bg);backdrop-filter:blur(24px);padding:24px;gap:20px;border-bottom:1px solid var(--border);z-index:199;animation:fadeIn 0.2s ease;}
    }
...
const Stars = ({ dark }) => {
  ...
  useEffect(() => {
    ...
    let raf;
    const draw = () => { ctx.clearRect(0, 0, w, h); stars.forEach(s => { s.a = Math.max(0.05, Math.min(1, s.a + s.da)); if (s.a <= 0.05 || s.a >= 1) s.da *= -1; s.y -= s.speed; if (s.y < -2) { s.y = h + 2; s.x = Math.random() * w; } ctx.beginPath(); ctx.arc(s.x, s.y, s.r, 0, Math.PI * 2); ctx.fillStyle = `rgba(${dark ? "180,210,255" : "59,130,246"},${s.a * (dark ? 0.55 : 0.25)})`; ctx.fill(); }); raf = requestAnimationFrame(draw); };
    draw();
    const onResize = () => { w = c.width = window.innerWidth; h = c.height = window.innerHeight; };
    window.addEventListener("resize", onResize);
    return () => { cancelAnimationFrame(raf); window.removeEventListener("resize", onResize); };
  }, [dark]);
  return <canvas ref={ref} style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", opacity: dark ? 0.5 : 0.3, transition: "opacity 0.4s" }} />;
};

const MeshBg = ({ dark }) => (
  <div style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", overflow: "hidden", transition: "all 0.4s" }}>
    <div style={{ position: "absolute", top: "5%", left: "15%", width: 700, height: 700, borderRadius: "50%", background: `radial-gradient(circle,rgba(59,158,255,${dark ? 0.07 : 0.06}) 0%,transparent 70%)`, animation: "orb1 14s ease-in-out infinite" }} />
    <div style={{ position: "absolute", top: "45%", right: "5%", width: 550, height: 550, borderRadius: "50%", background: `radial-gradient(circle,rgba(139,92,246,${dark ? 0.065 : 0.05}) 0%,transparent 70%)`, animation: "orb2 18s ease-in-out infinite" }} />
    <div style={{ position: "absolute", bottom: "5%", left: "35%", width: 450, height: 450, borderRadius: "50%", background: `radial-gradient(circle,rgba(6,182,212,${dark ? 0.05 : 0.04}) 0%,transparent 70%)`, animation: "orb3 22s ease-in-out infinite" }} />
    <div style={{ position: "absolute", inset: 0, backgroundImage: `linear-gradient(rgba(${dark ? "255,255,255" : "0,0,0"},0.012) 1px,transparent 1px),linear-gradient(90deg,rgba(${dark ? "255,255,255" : "0,0,0"},0.012) 1px,transparent 1px)`, backgroundSize: "64px 64px" }} />
  </div>
);
...
const ThemeToggle = ({ dark, setDark }) => (
  <button
    ...
    style={{
      ...
      background: dark ? "rgba(59,158,255,0.2)" : "rgba(0,0,0,0.1)",
      border: `1.5px solid ${dark ? "rgba(59,158,255,0.4)" : "rgba(0,0,0,0.15)"}`,
  ...
  >
    {/* Track icons */}
    <span style={{ position: "absolute", left: 6, fontSize: 11, opacity: dark ? 0 : 1, transition: "opacity 0.3s" }}>☀️</span>
    <span style={{ position: "absolute", right: 6, fontSize: 11, opacity: dark ? 1 : 0, transition: "opacity 0.3s" }}>🌙</span>
    {/* Knob */}
    <div style={{
      width: 20, height: 20, borderRadius: "50%",
      background: dark ? "var(--blue)" : "#fff",
      boxShadow: dark ? "0 0 8px rgba(59,158,255,0.6)" : "0 1px 4px rgba(0,0,0,0.2)",
    ...
    <section id="hero" style={{ minHeight: "100vh", display: "flex", alignItems: "flex-start", padding: "100px 48px 60px", maxWidth: 1200, margin: "0 auto" }}>
      <div style={{ flex: "0 0 58%", paddingRight: 48 }}>
        ...
        <div style={{ animation: "fadeUp 0.6s ease both", animationDelay: "0.2s", opacity: 0, marginBottom: 4 }}>
          <h2 style={{ fontSize: "clamp(40px,5.5vw,68px)", fontWeight: 700, letterSpacing: "-0.03em", lineHeight: 1.05 }}>Yash Gupta</h2>
        </div>
      ...
      <div style={{ flex: "0 0 42%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", paddingTop: 0, gap: 24, animation: "fadeIn 0.8s ease both", animationDelay: "0.4s", opacity: 0 }}>
        <div style={{ position: "relative" }}>
          <div style={{ width: 240, height: 240, borderRadius: "50%", background: "linear-gradient(135deg,#3b9eff,#8b5cf6,#06b6d4)", padding: 4, boxShadow: "0 0 60px rgba(59,158,255,0.4), 0 0 100px rgba(139,92,246,0.25)", animation: "float 5s ease-in-out infinite" }}>
            <img src={PHOTO} alt="Yash Gupta" style={{ width: "100%", height: "100%", borderRadius: "50%", objectFit: "cover", objectPosition: "center top", border: "4px solid var(--bg)" }} />
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, width: "100%", maxWidth: 280 }}>
          {stats.map((s, i) => (
            <div key={s.label} style={{ padding: "18px 14px", borderRadius: 12, border: "1px solid var(--border)", background: "var(--surface)", textAlign: "center", backdropFilter: "blur(12px)", animation: `float ${4 + i * 0.5}s ease-in-out infinite`, animationDelay: `${i * 0.3}s`, transition: "border-color 0.2s, box-shadow 0.2s" }}
              ...
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 26, fontWeight: 700, color: s.color, lineHeight: 1 }}><Counter target={s.val} suffix={s.suf} decimals={s.dec} /></div>
      ...
      <style>{`
        @media(max-width:900px){
          ...
          #hero > div:nth-child(2) { flex: unset !important; width: 100% !important; }
        }
      `}</style>
    </section>
  );
        ...
        <Reveal delay={80}>
          <GC style={{ padding: "30px", height: "100%" }} tilt>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 22 }}>
              {coding.map(s => (
                <a key={s.label} href={s.url} target="_blank" rel="noopener" style={{ textDecoration: "none", textAlign: "center", padding: "10px 6px", borderRadius: 10, border: "1px solid var(--border)", background: "var(--surface)", transition: "all 0.2s", cursor: "pointer" }}
                  ...
                  <div style={{ fontSize: "clamp(18px,2.5vw,26px)", fontWeight: 700, color: s.color, lineHeight: 1, fontFamily: "var(--font-mono)" }}><Counter target={s.n} suffix={s.suf} /></div>
            ...
            <div style={{ borderTop: "1px solid var(--border)", paddingTop: 18, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 40, fontWeight: 700, lineHeight: 1 }}><Counter target={500} suffix="+" /></div>
...
const Experience = () => {
  const jobs = [
    { role: "Software Developer Intern", company: "ARIES", full: "Aryabhatta Research Institute of Observational Sciences", period: "Aug 2024 – Dec 2024", location: "Nainital, India", color: "var(--blue)", icon: "🔭", tools: ["Python", "Odoo", "Linux", "PostgreSQL", "Agile", "ERP"], points: ["Developed ERP modules using Python (Odoo) to automate HR and inventory workflows for improved data handling", "Built dynamic reporting tools with auto summaries, charts, and activity logs for administrative insights", "Deployed solutions on Linux servers with rollback support and unit testing for stable delivery", "Collaborated in Agile sprint planning and backend architecture discussions for scalable system design"] },
    { role: "Java Developer Intern", company: "Technohacks Edutech", full: "Technohacks Edutech", period: "Jul 2024 – Aug 2024", location: "Remote", color: "var(--purple)", icon: "💻", tools: ["Java", "Swing", "OOP", "Event-Driven", "GUI"], points: ["Developed interactive desktop applications to strengthen core Java concepts and event-driven programming", "Implemented GUI-based programs using Swing, focusing on user input handling and dynamic interfaces", "Practiced OOP design principles and logic structuring for real-time response and modular code"] },
  ];
  return (
      ...
      <Reveal>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 20, marginBottom: 40 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {tags.map(t => <button key={t} onClick={() => setFilter(t)} style={{ padding: "7px 16px", borderRadius: 999, fontSize: 12, fontFamily: "var(--font-mono)", border: `1px solid ${filter === t ? "var(--blue)" : "var(--border)"}`, background: filter === t ? "rgba(59,158,255,0.12)" : "var(--surface)", color: filter === t ? "var(--blue)" : "var(--muted)", cursor: "pointer", transition: "all 0.2s" }}>{t}</button>)}
          </div>
        </div>
      </Reveal>
            ...
            <GC style={{ padding: "28px", height: "100%", display: "flex", flexDirection: "column", border: p.featured ? "1px solid rgba(59,158,255,0.2)" : undefined }} tilt>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  <span className="chip" style={{ fontSize: 10 }}>{p.period}</span>
  ...
  const Bar = ({ name, pct, color }) => {
    ...
    return (
      <div ref={ref} style={{ marginBottom: 11 }}>
        ...
        <div style={{ height: 4, background: "rgba(128,128,128,0.15)", borderRadius: 3, overflow: "hidden" }}>
      ...
      <Reveal delay={200} style={{ marginTop: 32 }}>
        <GC style={{ padding: "28px 32px" }}>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            {[{ icon: "🤝", title: "NSS Volunteer", desc: "Campus outreach & awareness drives" }, { icon: "🌱", title: "Eco Club", desc: "Waste-to-Best campaigns & clean campus" }, { icon: "📡", title: "URSI-RCRS 24 Organizer", desc: "Logistics & tech for IEEE conference" }].map(a => (
    ...
    <section id="contact" style={{ padding: "80px 48px 100px", maxWidth: 1200, margin: "0 auto" }}>
      <Reveal>
        <GC style={{ padding: "clamp(36px,6vw,72px)", textAlign: "center", background: "linear-gradient(135deg,rgba(59,158,255,0.04),rgba(139,92,246,0.04))", border: "1px solid rgba(99,179,255,0.13)", animation: "borderGlow 4s ease-in-out infinite" }}>
          ...
          <h2 style={{ fontSize: "clamp(30px,5.5vw,58px)", fontWeight: 700, letterSpacing: "-0.03em", marginBottom: 14, marginTop: 8 }}>Let's Build Something</h2>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginBottom: 48 }}>
            <button onClick={copy} className="btn-p" style={{ minWidth: 200 }}>
              {copied ? <><svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path d="M5 13l4 4L19 7" strokeLinecap="round" strokeLinejoin="round"/></svg>Copied!</> : <><svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" strokeLinecap="round" strokeLinejoin="round"/></svg>Copy Email</>}
            </button>
